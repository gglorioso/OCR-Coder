#!/usr/bin/env python3
"""
run_attention_probe.py — Attention Probe for MVV Phase 1.6 (Experiment A)

Architecture: AttentionProbe
  - MLP adapter: vision space (1152D) → LLM space (2048D)
  - Learnable CLS token
  - Standard MHA with NO positional encodings (Experiment A)
...
Output:
  MVV/Phase_1_6/results/attention_probe_results.json
"""
...
OUT_PATH    = _REPO_ROOT / "MVV" / "Phase_1_6" / "results" / "attention_probe_results.json"
...
class AttentionProbe(nn.Module):
    def __init__(self):
        ...
        # Standard MHA — NO positional encodings (Experiment A)
        self.attention = nn.MultiheadAttention(
            embed_dim=2048, num_heads=16, batch_first=True
        )
        # Regression head: CLS output → scalar
    ...
    def forward(self, x):
        # x: [B, N_tokens, 1152]
        x = self.adapter(x)                              # [B, N, 2048]
        cls = self.cls_token.expand(x.size(0), -1, -1)  # [B, 1, 2048]
        x = torch.cat([cls, x], dim=1)                  # [B, N+1, 2048]
        x, _ = self.attention(x, x, x)                  # [B, N+1, 2048]
        cls_out = x[:, 0, :]                             # [B, 2048]
...
def main() -> None:
    print("=" * 70)
    print("MVV Phase 1.6 — Attention Probe (Experiment A, no positional encoding)")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X_tensor)):
        ...
        model = AttentionProbe().to(device)
    ...
    output = {
        "experiment": "phase_1_6_attention_probe_experiment_A",
        "description": (
            "AttentionProbe (no positional encoding) on method2_pool8x8 features, "
            "target=n_defs"
        ),
        "architecture": {
            "input_dim": INPUT_DIM,
