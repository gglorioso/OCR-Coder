"""
Phase 3.4 -- Stage 1 Inference

Loads the Stage 1 checkpoint (ConvRoPEProjector + QLoRA LLM) and runs
greedy generation on random code-image tensors to evaluate reconstruction:

    SigLIP tensor [1024, 1152]
      -> ConvRoPEProjector [256, 2048]
      -> splice into token embeddings
      -> DeepSeek-Coder-V2-Lite-Instruct (4-bit QLoRA)
      -> greedy decode -> generated source text

Usage:
    python run_inference_stage1.py
    python run_inference_stage1.py --model-path /path/to/model --num-samples 5
"""
...
def load_model_for_inference(model_path: str, ckpt_dir: str, device: str):
    """Load tokenizer, 4-bit quantized LLM with LoRA weights, and ConvRoPEProjector."""
    ...
    # 4-bit quantization config -- must match training exactly
    ...
    print(f"[load] LLM in 4-bit (nf4) from {model_path}")
    ...
    # Load trained LoRA adapter directly (config is saved inside the adapter dir)
...
def generate(projector, llm, tokenizer, vision_tensor, device, max_new_tokens=MAX_GEN_TOKENS):
    ...
    Embedding construction:
        ...
        3. Cast to float16
        4. Replace first 256 positions with projected visual embeddings
        5. Generate with inputs_embeds (NOT input_ids)
    """
    ...
    # Build embeddings -- CRITICAL: must match training exactly
    # prepare_model_for_kbit_training casts embedding to fp32, so we cast back to fp16
    with torch.no_grad():
        text_embeds = llm.get_input_embeddings()(input_ids)
    text_embeds = text_embeds.clone().to(torch.float16)

    # Splice in visual embeddings
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Phase 3.4 -- Stage 1 Inference")
    parser.add_argument("--model-path", type=str,
                        default="/data/ai_club/DeepSeek-Coder-V2-Lite-Instruct",
                        help="Path to DeepSeek-Coder-V2-Lite-Instruct model")
    parser.add_argument("--ckpt-dir", type=str,
                        default="MVV/Phase_3/Phase_3_4/DGX_run/checkpoints/stage1_4gpu/epoch_step_4000",
    ...
    print("=" * 80)
    print("Phase 3.4 -- Stage 1 Inference")
    print("=" * 80)
    ...
    for i, stem in enumerate(selected, 1):
        ...
        print(f"  Vision tensor: {vision_tensor.shape}")
        ...
        if len(generated) > 3000:
            print(f"... (truncated, {len(generated)} total chars)")
