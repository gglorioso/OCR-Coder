"""
Phase 3.4 -- Stage 2 (Reasoning Fine-Tune) Training -- V100 QLoRA variant
...
NCCL BROADCAST timeouts.

Initializes from Stage 1 DGX checkpoint and fine-tunes on reasoning Q&A data.
...
Key differences from Stage 1:
  ...
  - Multi-chunk vision: project each chunk separately, then concatenate
    ...
    def __getitem__(self, idx: int) -> Optional[dict]:
        ...
        try:
            ...
            for tp in tensor_paths:
                ...
                # Cast to float32 for safety (will be cast to fp16 later)
                chunk = chunk.float()
...
def collate_fn(batch: list) -> Optional[dict]:
    ...
    # Find max chunks and max text-only length in this batch
    max_chunks = max(b["n_chunks"] for b in batch)
    max_visual_tokens = max_chunks * N_VISUAL_TOKENS
    max_text_only = max(b["input_ids"].size(0) - b["n_chunks"] * N_VISUAL_TOKENS for b in batch)
    total_len = max_visual_tokens + max_text_only
    ...
    for b in batch:
        ...
        # Right-pad text to match max total length (uniform across batch)
...
# ---------------------------------------------------------------------------
# Model loading with Stage 1 checkpoint (4-bit QLoRA)
# ---------------------------------------------------------------------------
def load_model(model_path: str, stage1_ckpt_dir: str, local_rank: int):
    """Load tokenizer, 4-bit quantized LLM with LoRA, ConvRoPEProjector,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 3.4 -- Stage 2 Reasoning Fine-Tune (V100 QLoRA)")
    ...
    parser.add_argument("--stage1-ckpt-dir", type=str,
                        default="MVV/Phase_3/Phase_3_4/DGX_run/checkpoints/stage1/epoch_best",
                        help="Path to Stage 1 best checkpoint directory "
                             "(containing projector.pth and lora_adapter/)")
    parser.add_argument("--save-dir", type=str,
                        default="MVV/Phase_3/Phase_3_4/DGX_run/checkpoints/stage2",
    ...
    if is_main_process():
        print(f"[main] Starting Stage 2 Reasoning Fine-Tune (V100 QLoRA)")
    ...
    model, tokenizer = load_model(args.model_path, args.stage1_ckpt_dir, local_rank)
    train(
        model, tokenizer,
