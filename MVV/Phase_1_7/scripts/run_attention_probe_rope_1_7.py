#!/usr/bin/env python3
"""
run_attention_probe_rope_1_7.py — Attention Probe for MVV Phase 1.7

Architecture: AttentionProbeRoPE (identical to Phase 1.6 Experiment B)
  - MLP adapter: vision space (1152D) → LLM space (2048D)
  - 2D SinCos positional encoding injected into spatial tokens
  - Learnable CLS token (no positional encoding applied to CLS)
  - Standard MHA with 2D sinusoidal positional encodings
  - Regression head: CLS output → scalar

Features: Method 2 pool8x8 from Phase 1.7 enhanced images
  - Each .pt file: [73728] fp16 = 64 tokens × 1152D
  - Reshaped to [1152, 8, 8].T → [64, 1152]

Target: n_defs (same as Phase 1.6 baseline)
Training: 5-fold CV, AdamW(lr=1e-4), MSE loss, 20 epochs per fold

CLI args:
  --feat-dir   Path to directory of .pt files (required)
  --out-path   Path for output JSON (required)
  --exp-name   Experiment name string for JSON (required)
"""
...
_REPO_ROOT  = _SCRIPT_DIR.parents[2]   # OCR-Coder/  (script is in MVV/Phase_1_7/scripts/)
...
# ---------------------------------------------------------------------------
# Hyperparameters  (identical to Phase 1.6)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model  (identical to Phase 1.6 run_attention_probe_rope.py)
# ---------------------------------------------------------------------------
...
class SinCos2DPositionalEncoding(nn.Module):
    ...
    def __init__(self, grid_h: int = 8, grid_w: int = 8, embed_dim: int = 2048):
        ...
        def make_1d_sincos(length, dim):
            ...
            enc[:, 1::2] = torch.cos(angle[:, :dim // 2 - (dim % 2)])
            return enc  # [L, dim]
...
class AttentionProbeRoPE(nn.Module):
    ...
    def forward(self, x):
        ...
        x   = self.pos_enc(x)                             # [B, 64, 2048]
        cls = self.cls_token.expand(x.size(0), -1, -1)    # [B, 1, 2048]
        x   = torch.cat([cls, x], dim=1)                  # [B, 65, 2048]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase 1.7: AttentionProbeRoPE on enhanced-image features"
    )
    ...
    parser.add_argument("--exp-name",  required=True,
                        help="Experiment name string used in output JSON")
    ...
    print("=" * 70)
    print(f"MVV Phase 1.7 — Attention Probe (2D SinCos Positional Encoding)")
    ...
    output = {
        ...
        "description": (
            f"AttentionProbeRoPE (2D SinCos positional encoding) on method2_pool8x8 features, "
            f"target=n_defs — {args.exp_name}"
        ),
        "architecture": {
            "input_dim": INPUT_DIM,
            "n_tokens":  N_TOKENS,
