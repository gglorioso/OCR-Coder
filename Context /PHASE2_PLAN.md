...
- 2026-04-06: **Teacher forcing identified as root cause of poor reconstruction.** 99.78% of tokens predictable from text context alone — model never learned to read vision tokens. Prefix-only training fix applied (mask text in input_ids with pad tokens). 21x embedding scale mismatch discovered (projector std=1.64 vs LLM std=0.08); LayerNorm + learnable output_scale (init=0.1) added to ConvRoPEProjector. Batch size 4→2 (OOM fix — 50% skip rate was discarding half the data). Fresh v2 training queued as `stage1_4h100_v2` with all fixes.
- 2026-03-21: **Phase 3.3 Fixed LR Training PASSED.** Job 237225: train_loss 1.51→0.93, val_loss 1.34→1.08 (9 epochs, killed at epoch 10 by time limit). Differential LR (lr_lora=5e-6, lr_proj=1e-5) eliminates divergence from Phase 3 lr=2e-4 bug. Pipeline validated end-to-end. Next: mass render 7,900 files, scale to DGX.
- 2026-02-24: **Contrastive loss identified as critical missing component.** Clean probe results (job 225980): Top-1=4.6%, Top-5=9.3%, 28.2× above random. Probe B unusable (all-positive labeling bug, not worth fixing). Root cause of retrieval failure: generation loss (cross-entropy on text tokens) does NOT directly optimize visual-text embedding alignment. Plan: add InfoNCE term `L_total = L_generation + 0.1 * L_InfoNCE(mean_pool(adapter_out), text_emb)`. Task redesign: drop function_listing/signatures, triple description/function_explanation, add domain_classification + has_import binary tasks. Priority: fix training objective BEFORE scraping more repos (more data with wrong loss produces same near-random retrieval at larger scale). Retrieval Recall@k replaces ROUGE-L as primary metric.
...
- 2026-02-26: **Phase 3 contrastive debugging — 4 runs, 3 bugs fixed.** (1) torchrun path fix; (2) txt_emb space mismatch fixed (hidden_states→embed_fn raw token embeddings); (3) batch_size 4→8 (contrastive N fix); (4) bias_init -10→-5 (sigmoid saturation fix); contrast_weight 0.1→0.5. phase2b_v4 result: val_pos_cos=0.419 flat (phase2a baseline, no contrastive learning). phase2b_v5 ready to submit.
